...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			local extraString = "\n" .. Config.Strings.TotalCost .. ": ~g~$~w~" .. Round(currentCost, 1)

			DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.CancelFuelingPump .. extraString)
			DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Round(currentFuel, 1) .. "~w~%")
		else
			DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Config.Strings.CancelFuelingJerryCan .. "\nGas can: ~g~" .. Round(GetAmmoInPedWeapon(ped, 883325847) / 4500 * 100, 1) .. "~w~% | Vehicle: ~g~" .. Round(currentFuel, 1) .. "~w~%")
		end
	...
	if IsEntityPlayingAnim(ped, "weapon@w_sp_jerrycan","fire", 3) then
		TaskPlayAnim(ped,"weapon@w_sp_jerrycan","fire_outro", 8.0, -8, -1, 49, 0, 0, 0, 0)
		ClearPedTasks(ped)
	else
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 100 and canFuel then
							if currentCash > 0 then
								...
								if IsControlJustReleased(0, 38) and isNearPump then
									isFueling = true
									...
									if IsControlJustReleased(0, 38) and canFuel then
										isFueling = true										
