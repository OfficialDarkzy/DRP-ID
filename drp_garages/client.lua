...
Citizen.CreateThread(function()
	...
	while true do
		...
		for _, garage in pairs(DRPGarages.Garages) do
			...
			if(GetDistanceBetweenCoords(pedCoords, garage.SpawnPoint.Pos.x, garage.SpawnPoint.Pos.y, garage.SpawnPoint.Pos.z, true) < 0.7) then
				isInMarker  = true
				this_Garage = garage
		...
		if DRPGarages.CarImpound then
			for _, data in pairs(DRPGarages.Impound) do
				...
				if(GetDistanceBetweenCoords(pedCoords, data.Impound.Pos.x, data.Impound.Pos.y, data.Impound.Pos.z, true) < 0.7) then
					isInMarker  = true
					this_Impound = data
